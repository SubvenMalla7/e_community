...
import DiscussionPostBar from '../components/blog/discussionPostBar.js'
import RecentNotification from '../components/recentNotification.js';
import DiscussionPost from "../components/blog/discussionpost.js";
    ...
    componentDidMount = () => {
        ...
        firebase.auth().onAuthStateChanged(async _users => {
            if (!_users)
                ...
                await firebase
                    .firestore()
                    .collection('discussion').orderBy('id', 'desc')
                        ...
                        <div className="container">
                            <div className="row">
                                ...
                                <div className="col-lg-6 order-1 order-lg-2">
                                    <DiscussionPostBar blog={this.state.editBlog} image={this.state.imageAsUrl} messageCount={this.state.messageCount} notiCount={this.state.notiCount} />

                                    {blog.map((_blog, i) => (
                                        < DiscussionPost key={i} id={_blog.id} title={_blog.title} body={_blog.body} image={_blog.image} profileimage={_blog.userImage} name={_blog.user} findBlog={this.findBlog} />))}
                                </div>
